...
function downloadImage(url, dest) {
  return new Promise((resolve, reject) => {
    let finalUrl = url;
    if (url.startsWith('http://')) finalUrl = url.replace('http:', 'https:');
    
    https.get(finalUrl, { headers: { 'User-Agent': 'Bot-Downloader/1.0 (contact@example.com)' }}, (res) => {
...
async function run() {
  for (const [filename, wikiFile] of Object.entries(failedFiles)) {
    ...
    if (!fs.existsSync(destPath)) {
      console.log(`Resolving API for: ${wikiFile}`);
      ...
      try {
        ...
        if (pageId === '-1' || !pages[pageId].imageinfo) {
           console.log(`-> Could not resolve ${wikiFile} in API`);
           continue;
        }
